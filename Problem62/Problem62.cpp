...
bool isDateOneAfterDateTwo(sDate DateOne, sDate DateTwo) {
	return !(isDateOneEqualDateTwo(DateOne, DateTwo) || isDateOneBeforeDateTwo(DateOne, DateTwo));
}

bool isTheLastDayInTheMonth(sDate Date) {
...
short CompareTwoDates(sDate DateOne, sDate DateTwo) {
	short result;
	(isDateOneEqualDateTwo(DateOne, DateTwo)) ? (result = 0) : (isDateOneAfterDateTwo(DateOne, DateTwo) ? result = 1 : result = -1);
	return result;
}

enDateCompare CompareTwoDatesWithEnums(sDate DateOne, sDate DateTwo) {
	enDateCompare result;
	result = (isDateOneEqualDateTwo(DateOne, DateTwo)) ? (enDateCompare::Same) : ((isDateOneAfterDateTwo(DateOne, DateTwo) ? (enDateCompare::After) : (enDateCompare::Before)));
	return result;
}
...
bool isPeriodsOverlapSecondVersion(sPeriod PeriodOne, sPeriod PeriodTwo) {
	return !((CompareTwoDatesWithEnums(PeriodOne.DateOne, PeriodTwo.DateTwo) == enDateCompare::After) || (CompareTwoDatesWithEnums(PeriodOne.DateTwo, PeriodTwo.DateOne) == enDateCompare::Before));
}

short CalculatePeriodeLength(sPeriod Period, bool IncludeEndDay = false) {
	return DifferenceBetweenDateOneAndDateTwo(Period.DateTwo, Period.DateOne, IncludeEndDay);
}

bool isDateWithinPeriod(sPeriod Period, sDate Date) {
	sPeriod TempPeriod{ Date, Date };
	return isPeriodsOverlapSecondVersion(Period, TempPeriod);
}

bool isDateWithinPeriodSecondVersion(sPeriod Period, sDate Date) {
	return (CompareTwoDatesWithEnums(Period.DateOne, Date) == enDateCompare::Before) && (CompareTwoDatesWithEnums(Period.DateTwo, Date) == enDateCompare::After);
}
...
short CountOverlapDays(sPeriod PeriodOne, sPeriod PeriodTwo) {
	...
	if (PeriodOneLength < PeriodTwoLength) {
		while (isDateOneBeforeDateTwo(PeriodOne.DateOne, PeriodOne.DateTwo)) {
			if (isDateWithinPeriod(PeriodTwo, PeriodOne.DateOne))
				OverlapDays++;
		...
		while (isDateOneBeforeDateTwo(PeriodTwo.DateOne, PeriodTwo.DateTwo)) {
			if (isDateWithinPeriod(PeriodOne, PeriodTwo.DateOne))
				OverlapDays++;
			PeriodTwo.DateOne = IncreaseDateByOne(PeriodTwo.DateOne);
		}
	return OverlapDays;
}

enum enValidDate { Valid, Invalid };

enValidDate CheckValidMonth(sDate Date) {
	return (Date.Month >= 1 && Date.Month <= 12) ? enValidDate::Valid : enValidDate::Invalid;
}

enValidDate CheckValidDay(sDate Date) {
	return (Date.Day >= 1 && Date.Day <= GetNumberOfDaysPerMonth(Date)) ? enValidDate::Valid : enValidDate::Invalid;
}
bool isValidDate(sDate Date) {
	return (CheckValidMonth(Date) == enValidDate::Valid && CheckValidDay(Date) == enValidDate::Valid);
}

int main() {
	cout << endl << "Period one : " << endl;
	sDate Date = ReadFullDate();
	isValidDate(Date) ? (cout << "Yes, Date is valid date.") : (cout << "No, Date is not valid date.");
	return 0;
}