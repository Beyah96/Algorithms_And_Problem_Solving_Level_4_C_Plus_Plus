...
enDateCompare CompareTwoDatesWithEnums(sDate DateOne, sDate DateTwo) {
	...
	(isDateOneEqualDateTwo(DateOne, DateTwo)) ? (result = enDateCompare::Same) : (isDateOneAfterDateTwo(DateOne, DateTwo) ? (result = enDateCompare::After) : (result = enDateCompare::Before));
	return result;
}
...
bool isDateWithinPeriod(sPeriod Period, sDate Date) {
	...
	return isPeriodsOverlap(Period, TempPeriod);
}
...
int main() {
	...
	(isDateWithinPeriodSecondVersion(Period, Date)) ? (cout << "Yes, date is within period.") : (cout << "No, date is not within period.");
	return 0;
}